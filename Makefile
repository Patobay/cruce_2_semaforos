...
Uso:
	...
	make $(SRCDIR_VERILOG)/<entidad>.v : Genera el archivo Verilog sintetizado de la entidad <entidad> en $(SRCDIR_VERILOG)/<entidad>.v
...
$(SRCDIR_VERILOG)/%.v: $(CONTENTFILE) | $(SRCDIR_VERILOG)
	cd $(WORKDIR) && ghdl -m $(VHDLFLAGS) $(*F)
	cd $(WORKDIR) && ghdl --synth $(VHDLFLAGS) --out=verilog $(*F) > ../$(SRCDIR_VERILOG)/$(*F).v
